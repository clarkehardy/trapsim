...
The script does not go through Fusion's export pipeline (whose API STL writer emits body-local coordinates, discarding occurrence transforms). Instead each body is tessellated in-process with Fusion's own high-quality tessellator, the triangles are placed in the assembly frame, and the binary STL is written directly in millimetres — the same coordinate frame `geometry.yaml`'s `bounds_mm` uses. The placement is chosen by verification rather than trust: a candidate placement is only written if its bounding box matches the body's assembly-world bounding box, so a wrong-frame (or wrong-unit, or wrong-body) export fails loudly instead of silently corrupting the simulation geometry. Each run appends a per-file report to `fusion_export_log.txt` in the simulation folder. Nothing in the design is modified.
